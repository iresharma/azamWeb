      ...
      <div class="column is-half">
        <div class="column is-9">
          ...
          <div>
            <div class="file has-name">
              <label class="file-label">
                <input
                  ...
                  name="pdfnote"
                  @change="
                    videoclass = $event.target.files[0];
                    uploadValue = 0;
                  "
                />
                <span class="file-cta">
            ...
            <input
              class="input"
              v-model="video.subtitle"
              placeholder="Vedio description"
              type="text"
            /><br /><br />
            ...
            <div style="margin: 10px;">
              <div style="display: flex; justify-content: space-between">
                <big class="muted-text"
                  >Progress: {{ uploadValuevideo.toFixed() + "%" }}</big
                >
                <small v-if="videoclass !== null" class="muted-text">{{
                  bytesToSize(videoclass.size)
                }}</small>
              </div>
              <progress
                class="progress is-success"
      ...
      if (this.video.title !== "" && this.video.subtitle !== "") {
        ...
        this.video.tid =
          localStorage.getItem("type") == "admin"
            ...
            : localStorage.getItem("tid");
        const storageRef = firebaseApp.storageBucket
          .ref(`videos/${this.videoclass.name}`)
          .put(this.videoclass);
        storageRef.on(
          `state_changed`,
          (snapshot) => {
